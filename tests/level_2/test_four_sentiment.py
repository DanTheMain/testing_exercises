...
def test__check_tweet_sentiment__return_good_with_more_good_words():
    good_word, good_word_num, bad_word, bad_word_num = "good", 3, "bad", 2
    text = " ".join(
        [good_word for _ in range(good_word_num)]
        + [bad_word for _ in range(bad_word_num)]
    )

    assert check_tweet_sentiment(text, {good_word}, {bad_word}) == "GOOD"


def test__check_tweet_sentiment__return_bad_with_more_bad_words():
...
def test__check_tweet_sentiment__return_none_with_equal_good_and_bad_word_counts():
    good_word, bad_word, word_num = "good", "bad", 3
    text = " ".join(
        [good_word for _ in range(word_num)] + [bad_word for _ in range(word_num)]
    )
...
def test__check_tweet_sentiment__return_none_for_equal_good_and_bad_words_count():
...
def test__check_tweet_sentiment__return_good_with_many_words_more_good_words():
...
def test__check_tweet_sentiment__return_none_with_no_good_bad_words():
...
def test__check_tweet_sentiment__return_none_with_no_words():
    assert (
        check_tweet_sentiment("", {"some", "other", "words"}, {"more, words"}) is None
    )


def test__check_tweet_sentiment__return_bad_with_more_bad_upper_case_words():
    good_word_count, bad_word_count = 4, 7
    bad_words = {"bad_" + str(i + 1) for i in range(bad_word_count)}
    good_words = {"good_" + str(i + 1) for i in range(good_word_count)}
    other_words = {"some", "other", "words"}
    text = " ".join(
        good_words.union({str(w).upper() for w in bad_words}).union(other_words)
    )
